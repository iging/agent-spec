...
- **Avoid Enums:** Do not use TypeScript `enum`s. _Rationale: They generate runtime boilerplate objects that increase bundle size and cannot be tree-shaken. Use union literal types (`type Status = 'idle' | 'loading' | 'success'`) or `as const` object maps, which are fully erased at compile time._
