...
> **Purpose:** Cross-cutting rules for any agent generating or modifying React Native or Expo code. Reference this file before writing mobile UI logic. Library deep-dives live in `skills/mobile-react-native/react-native-best-practices/` and are authoritative for Reanimated, Gesture Handler, SVG, Audio, JSI, and on-device AI specifics. Visual outcomes follow `ui-ux-principles.md`; token vocabulary follows `design-tokens.md`.
...
- **Off the JS Thread:** Every user-visible animation runs through Reanimated worklets or the native driver. Blocking the JavaScript thread with visible animations drops frames. Route to `skills/mobile-react-native/react-native-best-practices/references/animations/` for patterns.
