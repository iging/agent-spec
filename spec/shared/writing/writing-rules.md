...
- **Mechanical enforcement** (`scripts/audit-compliance.js`) applies the Always Banned list minus function words (`however`, `really`, `literally`, `actually`, `basically`, `very`, `just`, `probably`) plus the Extended Tier. It runs outside rule-reference directories (`skills/`, `shared/`, `core/`, `context/`, `docs/`) so normative files stay exempt from prose-style checks. Function-word bans remain doctrine-only because mechanical flagging floods technical documentation with false positives.
