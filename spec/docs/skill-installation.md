...
- **Authority:** Authoritative guidance for installation in projects. Complementary to `spec/docs/skill-standard.md` (which governs how skills are _written_) and `spec/runtime/shared.md` §4 (which governs how tools _load_ them); this file is the link between the two.
- **Must not define:** How to _author_ a skill (see `spec/docs/skill-standard.md`). Tool-specific file mechanics (see `spec/runtime/`). Any rule content — skills and modules are copied verbatim, never rewritten here.
...
cp spec/skills/design-engineering/01-foundations-and-systems/design-taste-spec-exporter.md \
...
_Table is best-known at time of writing; verify against each tool's current docs (`spec/runtime/shared.md` §3)._
...
- [ ] `spec/runtime/[tool].md` adapter consulted for the exact location your tool reads

## 6. Related Standards

| Section                                              | What it covers                                      |
| ---------------------------------------------------- | --------------------------------------------------- |
| [`spec/docs/getting-started.md`](getting-started.md) | Adoption quick-start (this is the deeper companion) |
| [`spec/docs/skill-standard.md`](skill-standard.md)   | Enterprise skill authoring standard                 |
| [`spec/runtime/shared.md`](../runtime/shared.md)     | Adapter contract + skills mapping                   |
