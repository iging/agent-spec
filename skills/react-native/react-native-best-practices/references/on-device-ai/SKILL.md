...
Production patterns for on-device AI in React Native and Expo using [React Native ExecuTorch](https://github.com/software-mansion/react-native-executorch).
...
- Memory is tightly constrained. `ExecuTorchLLM` preallocates memory for context. If your token usage approaches the window size, generation fails. You must track token usage using `useTokenizer`.
- The ExecuTorch runtime on Android operates synchronously over JNI in a background thread. Do not try to intercept individual generation steps synchronously from JS outside the hook's event handlers.
