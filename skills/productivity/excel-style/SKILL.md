...
- **Authority:** Owns the spreadsheet styling workflow. Where this skill conflicts with the core `xlsx` skill on matters of style, this skill wins.
- **Must not define:** The raw spreadsheet data or the xlsx generation mechanics (owned by the `xlsx` skill).
- **Normative base:** `shared/writing-rules.md`; `docs/anti-patterns.md`; `skills/_template/SKILL.md`; `docs/skill-standard.md`; `references/STYLE-GUIDE.md`; the `xlsx` skill.
...
- **Action:** Read `references/STYLE-GUIDE.md` for the configured rules on tabs, layout, frozen panes, and colors.
- **Input:** Style guide + the `xlsx` skill's openpyxl/recalc workflow.
- **Stop Condition:** None.
...
| 2 (Consult) | AP-16 (context dump) | Style guide is the single source for look; xlsx skill owns mechanics. |
...
- **Version:** 2.0.0
- **Changelog:**
  - `2.0.0` (2026-08-09) — Elevated to Tier 5 per `docs/skill-standard.md`. De-attributed the "personal" framing; fixed the broken absolute `/mnt/skills/public/xlsx/` path in `references/STYLE-GUIDE.md` to the portable `skills/productivity/xlsx/SKILL.md`. Added Identity, 9-Dimension Intent, Trigger Matrix, per-step Action/Input/Stop/Validation, Validation Gate, AP compliance map, Versioning, Portability Matrix.
