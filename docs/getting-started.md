...
`AGENTS.md` is the portable, single-file synthesis of `core/`. It contains no project-specific content, so it works unmodified in any repository.

## 3. Fill in the project templates

Copy the `context/` directory and complete each placeholder:
...
Use the adapter in `runtime/` for your tool. Each adapter documents the exact files and locations that tool reads:
