...
- **Authority:** Primary entry router for all capabilities under `modules/dev-workflow/`.
...
| 8   | Success Criteria | Request cleanly routed to correct sub-path under `modules/dev-workflow/`.                     |
...
- **Validation:** Target file exists under `modules/dev-workflow/`.
...
{
  "module": "dev-workflow",
  "target_path": "modules/dev-workflow/workflows/SKILL.md",
...
- [ ] Dev workflow request mapped to valid target under `modules/dev-workflow/`.
...
- **Version:** 2.0.0
- **Changelog:**
  - `2.0.0` — Module root router created for `modules/dev-workflow/`.
...
**Output:** Target `modules/dev-workflow/evaluate-pr-suggestions/SKILL.md`.
