...
- **Normative base:** `shared/writing/writing-rules.md`; `docs/anti-patterns.md`; `skills/_template/SKILL.md`; `docs/skill-standard.md`; `modules/prompt-engineering/prompts-by-category/learning/prompt-engineer-mentor.md`; `references/module-mapping-guide.md`.
...
- **Action:** Match the discovered project domain with relevant capability suites in `modules/` (such as `dev-workflow`, `design-engineering`, `mobile-react-native`, `autonomous-dev`, `enterprise-business`, `prompt-engineering`, `research-and-productivity`, `content-and-growth`).
...
- **Validation:** Selected skills exist in `modules/` and match project stack requirements.
...
- [ ] Recommended skills exist in `modules/` and match project scope.
...
**Output:** Audit detects Next.js 15, React 19, TypeScript, Tailwind CSS, Prisma. Recommends `modules/dev-workflow` (`plan-feature`, `api-endpoint-generator`, `database-migration`) and `modules/design-engineering` (`01-foundations-and-systems`). Scaffolds `context/PRD.md`, `context/ARCHITECTURE.md`, `context/SCHEMA.md`, and `context/DESIGN.md` with placeholder markers intact. Presents adaptation summary for approval before writing.
...
**Output:** Non-destructive audit detects Python, Django, React Native, Expo. Preserves existing codebase untouched. Recommends `modules/mobile-react-native` (`react-native-best-practices`, `expo-project-structure`) and `modules/dev-workflow` (`testing/write-a-test`, `workflows/code-inspection`). Scaffolds `.agents/` context rules and `context/RULES.md` to enforce project safety boundaries. Requests user confirmation before writing governance files.
