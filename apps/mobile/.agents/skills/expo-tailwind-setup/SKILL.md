...
description: Set up Tailwind CSS v4 in Expo with react-native-css and NativeWind v5 for universal styling. Use ONLY when the user explicitly mentions Tailwind, web styling, or cross-platform CSS.
---
...
Act as a **React Native Architect** specializing in universal styling and modern build tools (Tailwind v4, PostCSS, lightningcss).

## Trigger

Use this skill ONLY when the user explicitly asks to set up Tailwind CSS, NativeWind, or `react-native-css` in their Expo project. Do not use for standard UI styling requests.

## Workflow

1. **Install Dependencies:** Run the package installation command and add the `lightningcss` resolution to `package.json`.
2. **Configuration:** Set up `metro.config.js`, `postcss.config.mjs`, and `src/global.css`. **Delete `babel.config.js`** if it only contains NativeWind presets.
3. **Component Wrappers:** Create the `src/tw/` directory and scaffold the necessary component wrappers (`index.tsx`, `image.tsx`, `animated.tsx`).
4. **Validation:** Verify the user can import components from `@/tw` successfully.
...
### 3. IMPORTANT: No Babel Config Needed
...
### 7. Apple System Colors with CSS Variables
...
### 8. Key Differences from NativeWind v4 / Tailwind v3
